...
const CashbackModel = {
  ...
  cashbackStatus: async (user_id) => {
    const query =
      "SELECT * FROM user_cashbacks WHERE user_id = ? AND status = 'paid'";
    const [data] = await db.query(query, [user_id]);
  ...
  // MR
    ...
    getTotalMRPayouts: async (all = false, year = null, month = null) => {
    try {
      let query;
      let params = [];
