...
export async function nwpMemberDetails(user_id, status, start_date, end_date) {
  let sql = `
    SELECT 
      u.name,
      u.mobile,
      ...
      nwp.approved,
      nwp.is_completed,
      DATE_FORMAT(nwp.approved_at,'%Y-%m-%d %H:%i:%s') AS approved_at
    FROM nwp_users_package AS nwp
    LEFT JOIN users AS u ON nwp.user_id = u.user_id
    LEFT JOIN nwp_packages AS p ON nwp.package_id = p.id
    WHERE nwp.user_id = ?
  ...
  let rewardEarningsListSql = `
      SELECT 
        ...
        nwp.earned_for,
        nwp.earned_date,
        ROUND((nwp.total_amount * 0.05) * 400) AS package_amount,
  ...
  ON nwp.reward_id = nwpup.reward_id
      ...
      WHERE nwp.earned_date <= CURDATE()
        ...
        AND DATE(nwp.earned_date) BETWEEN ? AND ?
    `;

  const rewardparams = [user_id, start_date, end_date];
  ...
  let is_nwp_user = nwp_result.length > 0 ? 1 : 0;
...
export async function nwpMembersDetails(year = null, month = null) {
  ...
  const [earningsRows] = await db.query(
    ...
    SELECT
      ...
      SUM(CASE WHEN  type = 'reward' THEN total_amount ELSE 0 END) AS reward_amount
    FROM nwp_earnings
  ...
  const [payoutRows] = await db.query(
    `
    SELECT
      SUM(CASE WHEN status = 'paid' OR status = 'pending' THEN total_amount ELSE 0 END) AS total_paid_amount
    FROM nwp_earnings
  ...
  const [earningsRowsAll] = await db.query(
    `
     SELECT
      SUM(CASE WHEN type = 'monthly' THEN total_amount ELSE 0 END) AS overall_amount,
      SUM(CASE WHEN status = 'pending' AND type = 'reward' THEN total_amount ELSE 0 END) AS reward_amount
    FROM nwp_earnings`,
    [],
  );
...
`);
  ...
  return {
    ...
    total_payouts: data[0]?.total || 0,
    overall_income: earningsRowsAll[0]?.overall_amount || 0,
  };
}

// kamalesh

export async function nwpMembersDetailsAll() {}
