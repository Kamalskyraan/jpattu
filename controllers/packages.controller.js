...
export const GetKRPackages = async (req, res) => {
  try {
    const packages = await PackageModel.getKRPackages();
    res
      .status(200)
      .json({ packages: packages, message: "Packages fetched successfully" });
  } catch (error) {
...
export const AddPackageToKRUser = async (req, res) => {
  try {
    const result = validationResult(req);
    const { errors } = result;
    if (errors.length > 0) {
    ...
    const user = await UserModel.getUserKR(user_data.user_id);

    if (user.length === 0) {
      return res.status(200).json({ message: "User not found" });
    }
    const members = await UserModel.hasKRMembers(user_data.user_id);
    ...
    if (members.length !== 0) {
      ...
      const new_ids = await UserModel.addPackageToKRUser({ user_data, level });
      return res.status(201).json({ message: "Success", new_ids: new_ids });
    }
  } catch (error) {
    console.log(error);
