...
export const deleteTempTTUser = async (req, res) => {
  try {
    const id = req.params?.id;
    if (!id) {
      return res.status(400).json({
...
export const verifyRTStatus = async (req, res) => {
  const token = req.cookies.auth;
  if (!token) return res.status(401).json({ authenticated: false });
  const user = jwt.verify(token, process.env.TOKEN_SECRET);
  ...
  try {
    if (user.role === "user") {
      const [data] = await UserModel.getUserRT(user.user_id);
      if (!data.id) {
        res.cookie(
          "auth",
          {},
      ...
      const [data] = await TempUserModel.getRTUser(user.user_id);
      if (!data.id) {
        res.cookie(
          "auth",
          {},
...
export const NPRegisterUser = async (req, res) => {
  try {
    const data = req.body;
    const result = validationResult(req);
    if (!result.isEmpty()) {
    ...
    const user = await UserModel.getUserNameFromNP(data?.referral_id);
    ...
    if (user.length === 0 && admin.length === 0) {
      ...
      const id = await TempUserModel.addNPUser(data);
      res.status(201).json({ id: id, message: "Registered Successfully" });
    }
  } catch (error) {
    if (error.message === "referrer not found") {
...
export const getNPUserName = async (req, res) => {
  try {
    const { referral_id } = req.query || false;
    if (!referral_id) {
      return res.status(400).json({ message: "referral_id is required" });
    ...
    const data = await UserModel.getNPUserName(referral_id);
    if (data.length === 0) {
      const adminData = await AdminModel.getUserName(referral_id);
...
export const NPPaidProof = async (req, res) => {
  const image = req.file?.filename;
  try {
    const id = req.body?.id;
    const txn_id = req.body?.txn_id;
    ...
    const submitted = await TempUserModel.NPpaidProof({
      image,
      user_id: id,
      txn_id,
    });
    if (submitted) {
      const [result] = await TempUserModel.getNPUser(id);
      res.status(200).json({ result, message: "Proof submitted successfully" });
    } else {
      if (image) {
        const imagePath = path.join(
...
export const getNPUser = async (req, res) => {
  try {
    const { user_id } = req.params || false;
    if (user_id !== req.user_id && req.role !== "admin") {
      return res.status(403).json({ message: "Action cannot be done!" });
    ...
    const data = await UserModel.getUserNP(user_id);
    if (data.length === 0) {
      res.status(200).json({ message: "User not found" });
    } else {
      const updatedData = data.map((val) => {
...
export const getTempNPUser = async (req, res) => {
  try {
    const { user_id } = req.params || false;
...
export const updateNPUser = async (req, res) => {
  try {
    const data = req.body;
    ...
    data.role = req.role;
    const updated = await UserModel.updateNPUser(data);
...
export const verifyNPStatus = async (req, res) => {
  const token = req.cookies.auth;
  if (!token) return res.status(401).json({ authenticated: false });
  const user = jwt.verify(token, process.env.TOKEN_SECRET);
  ...
  try {
    if (user.role === "user") {
      const [data] = await UserModel.getUserNP(user.user_id);
      if (!data.id) {
        res.cookie(
          "auth",
          {},
      ...
      const [data] = await TempUserModel.getNPUser(user.user_id);
      if (!data.id) {
        res.cookie(
          "auth",
          {},
...
export const FSRegisterUser = async (req, res) => {
  try {
    const data = req.body;
    const result = validationResult(req);
    if (!result.isEmpty()) {
    ...
    const user = await UserModel.getUserNameFromFS(data?.referral_id);
    ...
    if (user.length === 0 && admin.length === 0) {
      ...
      const id = await TempUserModel.addFSUser(data);
      res.status(201).json({ id: id, message: "Registered Successfully" });
    }
  } catch (error) {
    if (error.message === "referrer not found") {
...
export const FSPaidProof = async (req, res) => {
  const image = req.file?.filename;
  try {
    const id = req.body?.id;
    const txn_id = req.body?.txn_id;
    ...
    const submitted = await TempUserModel.FSpaidProof({
      image,
      user_id: id,
      txn_id,
    });
    if (submitted) {
      const [result] = await TempUserModel.getFSUser(id);
      res.status(200).json({ result, message: "Proof submitted successfully" });
    } else {
      if (image) {
        const imagePath = path.join(
...
export const deleteTempFSUser = async (req, res) => {
  try {
    const id = req.params?.id;
    if (!id) {
      return res.status(400).json({
    ...
    const result = await TempUserModel.deleteFSUser(id);
    if (result > 0) {
      res.status(200).json({
        message: "id deleted successfully",
      });
...
export const getTempFSUser = async (req, res) => {
  try {
    const { user_id } = req.params || false;
...
export const getFSUserName = async (req, res) => {
  try {
    const { referral_id } = req.query || false;
    ...
    const data = await UserModel.getFSUserName(referral_id);
...
export const updateFSUser = async (req, res) => {
  try {
    const data = req.body;
...
export const getFSUser = async (req, res) => {
  try {
    ...
    const data = await UserModel.getUserFS(user_id);
